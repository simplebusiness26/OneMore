...
function sfxDing(){tone(660,.10,'sine',.04);tone(990,.14,'sine',.03,.07)}
...
function createFloor(n){
  const r=floorRng(n),d=clamp((n-1)/65,0,1),kind=chooseKind(n,r),f=baseFloor(kind,kind.toUpperCase());
  const addCoinTrail=(ys=[510,480,510])=>ys.forEach((y,i)=>f.coins.push(coin(125+i*42,y)));
  switch(kind){
    case'spikes':{f.name='SPIKES';const x=145+Math.floor(r()*35);f.hazards.push(spike(x,26+d*10));if(n>12&&r()>.45)f.hazards.push(spike(x+78,22));break}
    case'movingSpikes':{f.name='MOVING SPIKES';const h=spike(178,28);h.motion={axis:'x',range:42,speed:1.35+d*.85,phase:r()*6.28};f.hazards.push(h);if(n>30){const h2=spike(250,22);h2.motion={axis:'y',range:22,speed:1.55+d,phase:r()*6.28};f.hazards.push(h2)}break}
    case'movingPlatforms':{f.name='MOVING PLATFORMS';f.platforms=[platform(0,GROUND,105),platform(140,515,72,{motion:{axis:'y',range:24,speed:1.1+d*.45,phase:r()*6.28}}),platform(247,GROUND,113)];f.hazards.push({type:'pit',x:105,y:GROUND,w:142,h:80});f.coins.push(coin(176,475));break}
    ...
    case'disappear':{f.name='DISAPPEARING';f.platforms=[platform(0,GROUND,100),platform(112,520,62,{disappear:true,phase:0}),platform(190,500,62,{disappear:true,phase:.55}),platform(270,GROUND,90)];f.hazards.push({type:'pit',x:100,y:GROUND,w:170,h:80});break}
    case'laser':{f.name='LASER';f.laser={x:142,y:505,w:112,h:5,phase:r()*1.1,period:1.35-d*.2,onTime:.62};f.coins.push(coin(198,470));break}
    case'rotor':{f.name='ROTATING ARM';f.rotor={cx:207,cy:510,len:72,width:7,angle:r()*6.28,speed:2.0+d*.9};break}
    case'wall':{f.name='DON’T STOP';f.wall={x:-64,w:48,speed:83+d*11};f.hazards.push(spike(170,22));break}
    ...
    case'bounce':{f.name='BOUNCE';f.platforms=[platform(0,GROUND,145),platform(240,GROUND,120)];f.hazards.push({type:'pit',x:145,y:GROUND,w:95,h:80});f.pads.push({x:112,y:GROUND-8,w:28,h:8,power:470});f.coins.push(coin(195,430));break}
    ...
    case'double':{f.name='DOUBLE SPEED';f.speedMul=1.38;f.modifier='×2 SPEED';f.hazards.push(spike(205,22));break}
    case'mystery':{f.name='MYSTERY';const mods=[()=>{f.speedMul=1.22;f.modifier='RUSH'},()=>{f.gravityMul=1.18;f.jumpMul=1.08;f.modifier='HEAVY'},()=>{f.jumpMul=1.18;f.modifier='SPRING'}];pick(mods,r)();f.hazards.push(spike(175,22));break}
    case'bonus':{f.name='COIN FLOOR';f.modifier='BONUS';addCoinTrail([515,475,445,475,515]);break}
    case'boss':{f.name=`BOSS ${Math.floor(n/10)}`;f.modifier='BOSS FLOOR';f.hazards.push(spike(130,24),spike(250,24));const mh=spike(192,25);mh.motion={axis:'y',range:24,speed:1.45+d*.65,phase:r()*6.28};f.hazards.push(mh);if(n>=30)f.laser={x:160,y:488,w:78,h:5,phase:.2,period:1.55,onTime:.5};if(n>=50)f.rotor={cx:285,cy:505,len:43,width:6,angle:0,speed:2.15};break}
  }
  return f;
}
...
function startRun(which='endless'){ensureAudio();mode=which;floorNo=1;runCoins=0;deathQueued=false;runStart=performance.now();seedBase=mode==='daily'?hashSeed(new Date().toISOString().slice(0,10)+'|ONE_MORE_FLOOR'):(Math.random()*0xffffffff)>>>0;tutorialStep=0;hideAll();state='playing';ui.hud.classList.add('show');floor=createFloor(1);resetPlayer();transitionTimer=.45;floorBanner=.7;syncHud();if(!tutorialDone){ui.tutorial.classList.add('show');ui.tutorialText.textContent='TAP TO JUMP'}}
function nextFloor(){floorNo++;if(mode==='daily'&&floorNo>100){completeDaily();return}bankCoins+=runCoins;runCoins=0;save(STORE.coins,bankCoins);floor=createFloor(floorNo);resetPlayer();transitionTimer=.52;floorBanner=.72;syncHud();sfxDing();haptic(18);flash()}
...
function floorPlatformUpdate(p,t,dt){if(p.motion){const v=Math.sin(t*p.motion.speed+p.motion.phase)*p.motion.range;if(p.motion.axis==='x')p.x=p.baseX+v;else p.y=p.baseY+v}if(p.collapse&&p.collapseTimer>=0){p.collapseTimer+=dt;if(p.collapseTimer>.55)p.fall+=780*dt;p.y+=p.fall*dt}if(p.disappear){const cyc=(t+p.phase)%1.6;p.opacity=cyc<.92?1:cyc<1.15?1-(cyc-.92)/.23:cyc<1.42?0:(cyc-1.42)/.18;p.solid=p.opacity>.15}}
...
function update(dt){
  ...
  player.vx=92*floor.speedMul;if(player.jumpBuffer>0&&(player.onGround||player.coyote>0)){player.vy=-315*floor.jumpMul;player.onGround=false;player.coyote=0;player.jumpBuffer=0;sfxJump();haptic(8)}
  player.vy+=880*floor.gravityMul*dt;player.vy=Math.min(player.vy,660);player.x+=player.vx*dt;player.y+=player.vy*dt;player.onGround=false;player.landedPlatform=null;
...
function drawPlatforms(f){for(const p of f.platforms){if(p.y>=H+30)continue;ctx.save();ctx.globalAlpha=p.opacity??1;ctx.fillStyle=p.ceiling?'#242a35':'#e9edf5';ctx.fillRect(p.x,p.y,p.w,p.h);if(p.collapse&&p.collapseTimer>=0){ctx.fillStyle='#ffb95c';ctx.fillRect(p.x,p.y,p.w*clamp(p.collapseTimer/.55,0,1),3)}ctx.restore()}}
